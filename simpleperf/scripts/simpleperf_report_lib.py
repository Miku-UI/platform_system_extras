...
def _load_native_libsimpleperf_report(native_lib_path: Optional[str] = None) -> ct.CDLL:
    """Loads the native libsimpleperf_report library and its dependencies.

    This function finds and loads the required native library for reporting. On
    Windows, it also handles loading of dependent libraries like libwinpthread.

    Args:
        native_lib_path: An optional path to the native library. If not
            provided, the function will search for it in standard locations.

    Returns:
        A ctypes.CDLL object for the loaded native library.

    Raises:
        FileNotFoundError: If the main native library or its dependencies
            cannot be found.
    """
    # As the windows dll is built with mingw we need to load 'libwinpthread-1.dll'.
    if is_windows():
        windows_pthread_path = get_host_binary_path('libwinpthread-1.dll')
        if windows_pthread_path is None:
            raise FileNotFoundError('Could not find dependent library: libwinpthread-1.dll')
        ct.CDLL(windows_pthread_path)

    if native_lib_path is None:
        native_lib_path = get_host_binary_path('libsimpleperf_report.so')
        if native_lib_path is None:
            raise FileNotFoundError('Could not find native library: libsimpleperf_report.so')

    return ct.CDLL(native_lib_path)
...
class ReportLib(object):
    ...
    def __init__(self, native_lib_path: Optional[str] = None):
        self._lib = _load_native_libsimpleperf_report(native_lib_path)
...
class ReportUtilsStructure(ct.Structure):
    _fields_ = []
...
class ProtoFileReportLib:
    ...
    _report_sample_pb2 = None

    @staticmethod
    def is_supported_format(record_file: Path) -> bool:
        """ Check if the record file is in cmd_report_sample.proto format.
            Return True if it is, False otherwise.
        """
        with open(record_file, 'rb') as fh:
            data = fh.read(10)
            if data == b'SIMPLEPERF':
                return True
        return False

    @staticmethod
    def get_report_sample_pb2():
        if ProtoFileReportLib._report_sample_pb2 is None:
            try:
                import report_sample_pb2
                ProtoFileReportLib._report_sample_pb2 = report_sample_pb2
            except ImportError as e:
                log_exit(f'{e}\nprotobuf package is missing or too old. Please install it like ' +
                         '`pip install protobuf==4.21`.')
        return ProtoFileReportLib._report_sample_pb2

    def __init__(self, native_lib_path: str | None = None):
        self._lib = _load_native_libsimpleperf_report(native_lib_path)

        self._create_report_utils_func = self._lib.ReportUtils_Create
        self._create_report_utils_func.restype = ct.POINTER(ReportUtilsStructure)

        self._destroy_report_utils_func = self._lib.ReportUtils_Destroy
        self._destroy_report_utils_func.argtypes = [ct.POINTER(ReportUtilsStructure)]

        self._add_proguard_mapping_file_func = self._lib.ReportUtils_AddProguardMappingFile
        self._add_proguard_mapping_file_func.restype = ct.c_bool
        self._add_proguard_mapping_file_func.argtypes = [
            ct.POINTER(ReportUtilsStructure), ct.c_char_p]

        self._de_obfuscate_java_methods_func = self._lib.ReportUtils_DeObfuscateJavaMethods
        self._de_obfuscate_java_methods_func.restype = ct.c_bool
        self._de_obfuscate_java_methods_func.argtypes = [
            ct.POINTER(ReportUtilsStructure), ct.c_char_p, ct.c_char_p, ct.c_size_t]

        # original_name is an output parameter, for _DeobfuscateJavaMethods_c.
        self._original_name_buf = ct.create_string_buffer(4096)

        self._instance = self._create_report_utils_func()
        _check(not _is_null(self._instance), 'Failed to create ReportUtil instance')
        ...
        self.offcpu_samples = {}
        self.has_proguard_mapping = False

    def Close(self):
        """Releases the native report_util instance."""
        if self._instance:
            self._destroy_report_utils_func(self._instance)
            self._instance = None
    ...
    def AddProguardMappingFile(self, mapping_file: Union[str, Path]) -> None:
        """Add a Proguard mapping file to de-obfuscate method names.

        This method reads a Proguard mapping file and stores the de-obfuscation
        information. This information is then used by the `_DeObfuscateJavaMethods`
        method to translate obfuscated Java method names back to their original
        names.

        Args:
            mapping_file: The path to the Proguard mapping file. This can be
                either a string or a Path object.

        Raises:
            ValueError: If the mapping file cannot be added.
        """
        if not self._add_proguard_mapping_file_func(self._instance, _char_pt(str(mapping_file))):
            raise ValueError(f'failed to add proguard mapping file: {mapping_file}')
        self.has_proguard_mapping = True

    def _DeObfuscateJavaMethods(self, obfuscated_name: str) -> str:
        """De-obfuscates a Java method name using the Proguard mapping file.

        This method takes an obfuscated Java method name and, if a Proguard
        mapping file has been provided, attempts to find the original
        (de-obfuscated) name. If no mapping is found, or if no mapping file has
        been loaded, the original obfuscated name is returned.

        Args:
            obfuscated_name: The obfuscated Java method name to de-obfuscate.

        Returns:
            The de-obfuscated method name, or the original name if no mapping
            is found.
        """
        result = self._de_obfuscate_java_methods_func(self._instance, _char_pt(obfuscated_name),
                                                      self._original_name_buf,
                                                      len(self._original_name_buf))
        if result:
            return bytes_to_str(self._original_name_buf.value)
    ...
    def _build_symbol(self, node) -> ProtoSymbol:
        ...
        if node.symbol_id == -1:
            ...
            if self.has_proguard_mapping:
                symbol_name = self._DeObfuscateJavaMethods(symbol_name)
...
def GetReportLib(record_file: str) -> Union[ReportLib, ProtoFileReportLib]:
    if ProtoFileReportLib.is_supported_format(Path(record_file)):
